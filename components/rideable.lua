...
function Rideable:SetSaddle(doer, newsaddle)
    ...
    --print("inst: "..(self.inst.name or 'nil'))
    ...
    if newsaddle ~= nil then
        if self.saddleable then
            ...
            if self.saddle ~= nil then
                self.inst.components.lootdropper:FlingItem(newsaddle, pt, doer == nil and self.saddle.components.saddler.discardedcb or nil)
            else
                self.inst.components.lootdropper:FlingItem(newsaddle, pt, nil)
            end
